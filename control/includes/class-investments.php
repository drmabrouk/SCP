...
class Control_Investments {

	public static function add_investment( $data ) {
		global $wpdb;
		$table = $wpdb->prefix . 'control_investments';

		// Enforce strict investment limits
		$property = Control_Properties::get_property($data['property_id']);
		$perf = Control_Properties::get_property_performance($data['property_id']);
		$remaining = $perf['total_project_cost'] - $perf['total_invested'];

		if ( $data['amount'] > $remaining + 0.01 ) {
			return new WP_Error('overfunding', sprintf(__('المبلغ يتجاوز التمويل المطلوب. المتبقي: %s AED', 'control'), number_format($remaining)));
		}
		...
		if ( count($existing_investments) >= 3 && !in_array($data['investor_id'], wp_list_pluck($existing_investments, 'investor_id')) ) {
			return new WP_Error('investor_limit', __('تم الوصول للحد الأقصى لعدد الشركاء في هذا المشروع (3 شركاء).', 'control'));
		}
		...
		if ( $data['amount'] > 0 ) {
			self::record_transaction( $data['investor_id'], $data['amount'], 'investment', sprintf( __('مساهمة في مشروع: %d', 'control'), $data['property_id'] ) );
		}
	...
	public static function get_investor_properties( $investor_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "
			...
			FROM {$wpdb->prefix}control_properties p
			JOIN {$wpdb->prefix}control_investments i ON p.id = i.property_id
	...
	public static function get_wallet( $user_id ) {
		global $wpdb;
		$table = $wpdb->prefix . 'control_wallets';
	...
	public static function record_transaction( $user_id, $amount, $type, $description = '' ) {
		global $wpdb;
		$wpdb->insert( "{$wpdb->prefix}control_transactions", array(
			'user_id' => $user_id,
			'amount' => $amount,
			'type' => $type,
		...
		$wpdb->update( "{$wpdb->prefix}control_wallets", $update_data, array('user_id' => $user_id) );
	}
	...
	public static function get_transactions( $user_id ) {
		global $wpdb;
		return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}control_transactions WHERE user_id = %d ORDER BY transaction_date DESC LIMIT 50", $user_id ) );
	}

	public static function get_system_wide_stats() {
		global $wpdb;

		$total_invested = $wpdb->get_var( "SELECT SUM(amount) FROM {$wpdb->prefix}control_investments" ) ?: 0;
		$total_expenses = $wpdb->get_var( "SELECT SUM(amount) FROM {$wpdb->prefix}control_expenses" ) ?: 0;

		// Sum of all real paid payments
		$total_revenue  = $wpdb->get_var( "SELECT SUM(amount) FROM {$wpdb->prefix}control_payments WHERE status = 'paid'" ) ?: 0;

		// Room stats
		$total_rooms = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}control_rooms" ) ?: 0;
		$occupied_rooms = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}control_rooms WHERE status = 'rented'" ) ?: 0;

		// Contract stats
		$active_contracts = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}control_contracts WHERE status = 'active'" ) ?: 0;
		$expired_contracts = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}control_contracts WHERE status = 'expired'" ) ?: 0;
		...
		$prev_revenue = $wpdb->get_var( $wpdb->prepare("SELECT SUM(amount) FROM {$wpdb->prefix}control_payments WHERE status = 'paid' AND payment_date BETWEEN %s AND %s", $prev_month_start, $prev_month_end) ) ?: 0;
		$growth_rate = ($prev_revenue > 0) ? (($total_revenue - $prev_revenue) / $prev_revenue) * 100 : 0;

		// Top/Bottom performing properties
		$all_props = $wpdb->get_results("SELECT id, name FROM {$wpdb->prefix}control_properties");
		foreach($all_props as &$p) {
			$p->perf = Control_Properties::get_property_performance($p->id);
		}
		...
		return array(
			...
			'investor_count' => $wpdb->get_var( "SELECT COUNT(DISTINCT id) FROM {$wpdb->prefix}control_staff WHERE role = 'investor'" ) ?: 0,
	...
	public static function distribute_proportional_amount( $property_id, $amount, $type, $description = '' ) {
		global $wpdb;
		$property = Control_Properties::get_property($property_id);
		if ( ! $property ) return;
		...
		if ( $owner_contribution > 0 ) {
			...
			self::record_transaction( $property->owner_id, abs($owner_share), $type, $description . sprintf(' (%s%% %s)', round($owner_share_percent * 100, 2), __('حصة المالك', 'control')) );
		}
	}

	public static function distribute_revenue( $property_id, $net_profit ) {
		self::distribute_proportional_amount($property_id, $net_profit, 'dividend', sprintf(__('توزيع أرباح يدوية - عقار #%d', 'control'), $property_id));
	}
	...
	public static function release_monthly_profits() {
		...
		$leased_properties = $wpdb->get_results("SELECT id, name, owner_id, base_value, contract_duration FROM {$wpdb->prefix}control_properties WHERE property_type = 'leased'");
			...
			if ($p->contract_duration > 0) {
				...
				if ($monthly_obligation > 0) {
					...
					self::distribute_proportional_amount(
						...
						$monthly_obligation,
						'lease_obligation',
						sprintf(__('استقطاع التزام عقد الإيجار الشهري - %s', 'control'), $p->name)
					);
				}
			}
		}
		...
		Control_Audit::log('profit_release', "Processed monthly obligations and released pending balances to available for withdrawal.");
	}
}
